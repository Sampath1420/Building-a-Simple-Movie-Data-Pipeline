...
def clean_omdb_currency(currency_str):
    """Convert OMDb BoxOffice string like '$120,000,000' to a numeric float."""
    if pd.isna(currency_str) or currency_str == 'N/A':
        return None
    try:
...
def clean_omdb_runtime(runtime_str):
    """Convert runtime string like '142 min' to integer (e.g., 142)."""
    ...
    try:
        ...
        if data.get('Response') == 'True':
            return {
                ...
                'plot': data.get('Plot'),
                'box_office': clean_omdb_currency(data.get('BoxOffice')),
                'poster_url': data.get('Poster'),
                'runtime_minutes': clean_omdb_runtime(data.get('Runtime')),
...
def etl_pipeline():
    ...
    expected_cols = [
        ...
        'plot', 'box_office', 'poster_url', 'runtime_minutes', 'metascore'
    ]
